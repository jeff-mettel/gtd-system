...
Goal: run Snowball on real data, empty from day one, with Google Calendar
and Claude live, and with user data preserved across every future release.

## Principles

1. **Data lives outside the app.** `~/Snowball/` (a private git repo) holds
...
3. **One writer.** The local service owns the ledger file. Front-end, `snow` CLI and
...
| **2 — Claude live** | `server/` (Node): serves the build, JSON API over the ledger, spawns `claude -p` jobs; `bin/snow` CLI as a thin client; skills + subagents per `subscription-wiring.md` (clarify, suggest, draft nudge, prep, review prep, wiki compile/ingest) with tiers from Settings; scheduled runs; harness deny rules + guard; Delegated lane shows real runs. |
...
- Data folder `~/Snowball/`, private GitHub repo.
