...
8. **Replay** — inside Flow: the ledger played back over time as balls moving through
   the system (`frontend/src/replay`); any date is a fold of the event log, so the other
   views can be rewound too. The weekly review links to it scoped to the week.
