# Desktop app — Delivery System.app
...
```
Delivery System.app/Contents/
  MacOS/gtd-desktop          the Rust launcher (window, tray, shortcut, updater)
...
1. picks the data folder — `--data <dir>` on the command line, else `GTD_DATA`, else `~/GTD-data`
   (the same folder the runbook uses; the app never writes anywhere else except its own window
   state under `~/Library/Application Support/com.jeffmettel.deliverysystem/`; the sidecar's
...
2. probes for a free port from 4310 upward and writes it to `<data>/.port` (removed on quit) so
   the CLI can find the app's server: `gtd` with `GTD_SERVER=http://127.0.0.1:$(cat ~/GTD-data/.port)`;
3. spawns `MacOS/node Resources/server/index.js` with `PORT`, `GTD_DATA`, `GTD_APP_ROOT`, `HOME` and a
   `PATH` that starts with `Contents/MacOS` (so `#!/usr/bin/env node` in `bin/gtd` resolves inside
...
| Item | Does |
|---|---|
| Open Delivery System | shows the window |
| New capture (⌘⇧K, global) | shows the window and opens the capture overlay (`window.__gtdOpenCapture`, exported by `frontend/src/features/captureOverlay.js`) |
...
| Quit Delivery System | stops the sidecar and exits |
...
Unchanged: `~/GTD-data/` (`ledger/events.jsonl`, `wiki/`, `attachments/`, `config.json`, `runs/`).
Upgrading the app never touches it. Settings → Data (backup, export, import) works exactly as in
the browser. To run the app against a scratch folder: `"/Applications/Delivery System.app/Contents/MacOS/gtd-desktop" --data /tmp/gtd-scratch`.
...
**An update never touches `~/GTD-data`.** Only the `.app` bundle is replaced; ledger, wiki,
...
`https://github.com/jeff-mettel/gtd-system/releases/latest/download/latest.json`, which the release
workflow uploads next to the dmg (`includeUpdaterJson`) together with the signed
`Delivery System.app.tar.gz` + `.sig`. Signatures are checked against the public key in
...
When a newer version exists a native dialog says "Delivery System X is available — Update now /
...
in `~/GTD-data/config.json` (any other keys stay as they are), or export `GTD_GITHUB_TOKEN` before
...
clean alternatives are making the repo public or publishing releases to a public `gtd-releases` repo.

Manual fallback at any time: download `Delivery-System-macOS.dmg` from the README link and drag
...
open desktop/src-tauri/target/release/bundle/macos/     # Delivery System.app
ls   desktop/src-tauri/target/release/bundle/dmg/       # Delivery System_<version>_aarch64.dmg + Delivery-System-macOS.dmg
```
...
GitHub Release with tauri-action's `Delivery System_<version>_aarch64.dmg`, a copy named
`Delivery-System-macOS.dmg`, and the updater set (`Delivery System.app.tar.gz`, `.sig`,
...
`https://github.com/jeff-mettel/gtd-system/releases/latest/download/Delivery-System-macOS.dmg`, which
