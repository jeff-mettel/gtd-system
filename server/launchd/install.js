...
// Env honoured: GTD_DATA, PORT. Node and claude must be reachable from the PATH baked into the plist.
...
const LABEL = 'com.jeffmettel.gtd';
...
if (process.argv.includes('--uninstall')) {
  ...
  console.log(`[gtd] unloaded ${LABEL} and removed ${dest}`);
  process.exit(0);
}
...
if (!claudeDir) console.warn('[gtd] warning: `claude` is not on PATH; jobs will fail until it is (the plist bakes in the current PATH)');
...
console.log(`[gtd] wrote ${dest}\n[gtd] data ${data} · port ${process.env.PORT || 4310}\n[gtd] launchctl bootstrap: ${out.trim() || 'ok'}\n[gtd] logs: ${path.join(data, 'runs', 'server.log')}`);
