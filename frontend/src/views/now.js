...
import { backChip, backFirst, isBackToday, untilStart } from '../features/defer.js';
...
/* A shared row with the "back today" chip in front of its meta. Until backChip is wired into actionRow itself
   (ui/fragments.js), the chip is spliced in here. */
const rowWithBack = (a) => isBackToday(a) ? actionRow(a).replace('<div class="m">', `<div class="m">${backChip(a)}`) : actionRow(a);
...
export function viewNow() {
  ...
  ${(() => { const days7 = [...Array(7)].map((_, i) => d(i)); return `<div class="panel"><div class="ph"><h2>Hard landscape · next 7 days</h2><div style="display:flex;gap:10px;align-items:center"><span class="note">${state.collapsed.strip ? '' : 'Meetings and day-specific actions — click a meeting today for its prep brief. Ticklers resurface into the inbox on their day.'}</span><button class="btn sm ghost" data-collapse="strip">${state.collapsed.strip ? 'Expand' : 'Collapse'}</button></div></div>
    ${state.collapsed.strip ? '' : `<div class="cal week">${days7.map((day, i) => {
    ...
    <div class="panel"><div class="ph"><h2>Next actions by ${byProg ? 'program' : 'context'}</h2><span class="seg" style="margin-left:6px"><button data-group="context" class="${byProg ? '' : 'on'}">by context</button><button data-group="program" class="${byProg ? 'on' : ''}">by program</button></span><span class="sp" style="flex:1"></span><span class="note num">${actions.length} shown · ${delegated().length} with AI${parked.length ? ` · ${parked.length} deferred` : ''}</span></div>
