...
export function viewInbox() {
  ...
  <div class="inbox">
    <div class="panel ilist"><div class="ph"><h2>To clarify</h2><span class="note num">${inbox.length} items · ${inbox.filter(i => i.p.conf < .75).length} flagged</span></div><div class="note lhelp">Newest first · flagged = the AI wasn't sure</div>
      <div class="pb">${inbox.map(i => `<div class="row ${i.id === ui.sel ? 'sel' : ''}" data-sel="${i.id}" tabindex="${i.id === ui.sel ? 0 : -1}"><div class="conf ${i.p.conf < .75 ? 'low' : ''}" title="AI confidence ${Math.round(i.p.conf * 100)}%"><i style="width:${Math.round(i.p.conf * 100)}%"></i></div>${i.image ? `<img class="thumb" src="${i.image}" alt="" aria-hidden="true">` : ''}<div class="t"><div class="raw">${esc(i.raw)}</div><div class="m"><span class="chip src">${srcLabel[i.source]}${i.from ? ' · ' + esc(pname(i.from)) : ''}</span><span class="faint">${days(i.captured) === 0 ? 'today' : days(i.captured) + 'd ago'}</span><span class="chip">${kinds.find(k => k[0] === i.p.kind)?.[1] ?? i.p.kind}</span></div></div></div>`).join('')}</div>
    </div>
