...
import { activeRuns, isServer, people, projects } from '../store.js';
...
export function viewInbox() {
  const inbox = by('inbox').sort((a, b) => new Date(b.captured) - new Date(a.captured));
  if (!inbox.length) return `<div class="vhead"><div><h1>Inbox</h1><p>Every ask from every surface lands here. AI proposes what each one is; you confirm in one keystroke.</p></div></div><div class="panel"><div class="empty"><b>Inbox zero</b>Nothing waiting to be clarified. Capture something above.</div></div>${trashPanel()}`;
  if (!inbox.find(i => i.id === ui.sel)) ui.sel = inbox[0].id;
  const it = inbox.find(i => i.id === ui.sel), p = proposalOf(it);
  ...
  <div class="inbox">
    <div class="panel ilist"><div class="ph"><h2>To clarify</h2><span style="display:flex;gap:8px;align-items:center"><span class="note num">${inbox.length} items · ${inbox.filter(i => proposalOf(i).conf < .75).length} flagged</span>${isServer() ? (activeRuns().some(r => r.job === 'clarify') ? '<span class="chip aichip">AI clarifying…</span>' : `<button class="btn sm" data-runjob="clarify" title="Run the clarify job now (it also runs after each capture and every 15 minutes)">Clarify now</button>`) : ''}</span></div><div class="note lhelp">Newest first · flagged = the AI wasn't sure</div>
      <div class="pb">${inbox.map(i => `<div class="row ${i.id === ui.sel ? 'sel' : ''}" data-sel="${i.id}" tabindex="${i.id === ui.sel ? 0 : -1}"><div class="conf ${proposalOf(i).conf < .75 ? 'low' : ''}" title="AI confidence ${Math.round(proposalOf(i).conf * 100)}%"><i style="width:${Math.round(proposalOf(i).conf * 100)}%"></i></div>${i.image ? `<img class="thumb" src="${i.image}" alt="" aria-hidden="true">` : ''}<div class="t"><div class="raw">${esc(i.raw)}</div><div class="m"><span class="chip src">${srcLabel[i.source]}${i.from ? ' · ' + esc(pname(i.from)) : ''}</span><span class="faint">${days(i.captured) === 0 ? 'today' : days(i.captured) + 'd ago'}</span>${proposalOf(i).pending ? (isServer() ? '<span class="chip aichip">awaiting AI</span>' : '<span class="chip">first guess</span>') : `<span class="chip">${kinds.find(k => k[0] === proposalOf(i).kind)?.[1] ?? proposalOf(i).kind}</span>`}</div></div></div>`).join('')}</div>
    </div>
    <div class="panel detail">
      <div class="src"><div class="eyebrow">${srcLabel[it.source]}${it.from ? ' from ' + esc(person(it.from)?.name) : ''} · ${fmtDate(it.captured)}</div>
        <div class="confline ${low ? 'low' : ''}" title="How sure the AI is about its proposal below"><div class="conf ${low ? 'low' : ''}"><i style="width:${Math.round(p.conf * 100)}%"></i></div><span>${p.pending && isServer() ? (activeRuns().some(r => r.job === 'clarify') ? 'The AI is clarifying this now — the proposal will replace this first guess in a few seconds.' : 'Not clarified yet — this is a first guess. <button class="btn sm" data-runjob="clarify">Clarify with AI now</button>') : `${low ? 'Flagged · ' : ''}${Math.round(p.conf * 100)}% · ${esc(p.why)}`}</span></div>
        ${it.image ? `<img class="shot" src="${it.image}" alt="Pasted screenshot">` : ''}<div class="raw">${esc(it.raw)}</div></div>
      <div class="prop">
        ...
        ${p.kind === 'waiting' ? `<label>From</label><select id="pOwner">${people.map(x => `<option value="${x.id}" ${x.id === (p.owner || it.from) ? 'selected' : ''}>${esc(x.name)}</option>`).join('')}${people.length ? '' : '<option value="">— add people first —</option>'}</select>` : ''}
        ...
        ${p.kind === 'program' ? '' : `<label for="pStart" title="Tickler: leaves your lists until this day, then comes back at the top">Defer until</label>${dateField('pStart', p.start ? iso(p.start) : '')}`}
