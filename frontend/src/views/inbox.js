...
export function viewInbox() {
  ...
  <div class="inbox">
    <div class="panel ilist"><div class="ph"><h2>To clarify</h2><span class="note num">${inbox.length} items · ${inbox.filter(i => proposalOf(i).conf < .75).length} flagged</span></div><div class="note lhelp">Newest first · flagged = the AI wasn't sure</div>
      <div class="pb">${inbox.map(i => `<div class="row ${i.id === ui.sel ? 'sel' : ''}" data-sel="${i.id}" tabindex="${i.id === ui.sel ? 0 : -1}"><div class="conf ${proposalOf(i).conf < .75 ? 'low' : ''}" title="AI confidence ${Math.round(proposalOf(i).conf * 100)}%"><i style="width:${Math.round(proposalOf(i).conf * 100)}%"></i></div>${i.image ? `<img class="thumb" src="${i.image}" alt="" aria-hidden="true">` : ''}<div class="t"><div class="raw">${esc(i.raw)}</div><div class="m"><span class="chip src">${srcLabel[i.source]}${i.from ? ' · ' + esc(pname(i.from)) : ''}</span><span class="faint">${days(i.captured) === 0 ? 'today' : days(i.captured) + 'd ago'}</span><span class="chip">${kinds.find(k => k[0] === proposalOf(i).kind)?.[1] ?? proposalOf(i).kind}</span></div></div></div>`).join('')}</div>
    </div>
    <div class="panel detail">
      <div class="src"><div class="eyebrow">${srcLabel[it.source]}${it.from ? ' from ' + esc(person(it.from)?.name) : ''} · ${fmtDate(it.captured)}</div>
        <div class="confline ${low ? 'low' : ''}" title="How sure the AI is about its proposal below"><div class="conf ${low ? 'low' : ''}"><i style="width:${Math.round(p.conf * 100)}%"></i></div><span>${low ? 'Flagged · ' : ''}${Math.round(p.conf * 100)}% · ${esc(p.why)}</span></div>
