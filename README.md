...
Commitment management for program managers: snowflakes of work gathered into snowballs of execution. Capture from anywhere, let the AI clarify, keep the decisions yourself.

![Engage — what needs you now, then next actions by context](docs/screenshots/engage.png)

## Download

[![Latest release](https://img.shields.io/github/v/release/jeff-mettel/snowball?include_prereleases)](https://github.com/jeff-mettel/snowball/releases/latest)

**macOS (Apple Silicon): [Snowball-macOS.dmg](https://github.com/jeff-mettel/snowball/releases/latest/download/Snowball-macOS.dmg)** — always the newest build · [all releases](https://github.com/jeff-mettel/snowball/releases/latest)

First launch: the app is not notarized yet, so **right-click → Open** (once) instead of double-clicking. Your data lives in `~/Snowball`, outside the app; see [docs/desktop.md](docs/desktop.md).
...
| **Weekly review — reflect** · the keystone habit of the method with the evidence pre-gathered: inbox to zero, mind sweep, calendar sweeps, every project covered, aged waiting-fors, someday, wins, AI audit, wiki lint, horizons. | ![Weekly review](docs/screenshots/review.png) |
...
| `bin/snow` | CLI — thin client of the server; the write path for Claude jobs (`SNOW_ACTOR` required). `bin/snow-hook` is the PreToolUse guard. |
| `.claude/agents`, `.claude/skills` | The Claude jobs (`snow-clarify`, `snow-suggest`, `snow-nudge`, `snow-prep`, `snow-review`, `snow-compile`, `snow-ingest-calendar`) and the harness deny rules. |
...
| `docs/architecture.md` | The architecture: method mapping, data model, capture, clarify engine, views, agents, trust invariants, phasing. |
...
| `docs/subscription-wiring.md` | The same back-end on a Claude subscription instead of an API key: skills + subagents per tier, the `snow` CLI as the single write path, harness-enforced trust rules. |
...
| `docs/method-gap-review.md` | Node-by-node gap review of the prototype against the workflow the method describes. |

## Inspiration

Snowball is an independent open-source tool inspired by the personal productivity concepts described by David Allen in *Getting Things Done*. It is not affiliated with, approved by, or endorsed by David Allen or the David Allen Company.

## Status

Beta foundation: event-sourced front-end over a local service and the `packages/ledger` contract; Claude jobs run under your subscription via `claude -p`; Google Calendar via macOS Calendar or the connector. See `docs/runbook-beta.md` to run it on real data (empty start, data in `~/Snowball`).
...
Then open http://localhost:4310 (data in `~/Snowball`, created on first run). For front-end work: `cd frontend && npm run dev` (http://localhost:5173, `?demo=1` for example data). Everything is client-side; "Reset demo data" on the
